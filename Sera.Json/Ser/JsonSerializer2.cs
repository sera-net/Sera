...
public class JsonSerializer2(SeraJsonOptions options, AJsonFormatter formatter, AJsonWriter writer) : ASeraVisitor<Unit>
{
    ...
    public override Unit VReference<V, T>(V vision, T value)
    {
        ...
        return vision.Accept<Unit, JsonSerializer2>(this, value);
    }
    ...
    public override Unit VArray<V, T>(V vision, ReadOnlySequence<T> value)
    {
        ...
        foreach (var mem in value)
        {
            foreach (var item in mem.Span)
            {
                ...
                vision.Accept<Unit, JsonSerializer2>(this, item);
            }
        }
        writer.Write("]");
        state = last_state;
    ...
    public override Unit VSome<V, T>(V vision, T value)
        => vision.Accept<Unit, JsonSerializer2>(this, value);
    ...
    public override Unit VEntry<KV, VV, IK, IV>(KV keyVision, VV valueVision, IK key, IV value)
    {
        ...
        state = JsonSerializerState.None;
        writer.Write("[");
        keyVision.Accept<Unit, JsonSerializer2>(this, key);
        writer.Write(",");
        valueVision.Accept<Unit, JsonSerializer2>(this, value);
        writer.Write("]");
        state = last_state;
        return default;
    }
    ...
    private class MapSeraVisitor(JsonSerializer2 Base) : AMapSeraVisitor<Unit>(Base)
    {
        public override Unit VEntry<KV, VV, IK, IV>(KV keyVision, VV valueVision, IK key, IV value)
        {
            ...
            Base.writer.Write(":");
            return valueVision.Accept<Unit, JsonSerializer2>(Base, value);
        }
