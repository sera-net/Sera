    ...
    where N : ISeraCtor<T>
{
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public T Builder(string? name, Type<T> b = default) => ctor.Ctor();

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public R CollectField<R, C>(ref C colctor, int field, string? name, long? key, Type<T> b = default)
        where C : IStructSeraColctor<T, R>
        => colctor.CNone();
}
