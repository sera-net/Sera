...
public readonly struct SeqIImmutableListImpl<T, I, D, N>(D d, N ctor) :
    ISeraColion<T>, ISeqSeraColion<T, I>
    where T : IImmutableList<I>
    ...
    where N : ICapSeraCtor<T>
{
    ...
    public R CollectItem<R, C>(ref C colctor, Type<T> b = default) where C : ISeqSeraColctor<T, I, R>
        => colctor.CItem(d, new SeqIImmutableListEffector<T, I>());
}

public readonly struct SeqIImmutableListEffector<T, I> : ISeraEffector<T, I>
    where T : IImmutableList<I>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public void Effect(ref T target, I value) => target = (T)target.Add(value);
}
