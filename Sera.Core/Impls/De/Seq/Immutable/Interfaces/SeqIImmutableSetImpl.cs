...
public readonly struct SeqIImmutableSetImpl<T, I, D, N>(D d, N ctor) :
    ISeraColion<T>, ISeqSeraColion<T, I>
    where T : IImmutableSet<I>
    ...
    where N : ICapSeraCtor<T>
{
    ...
    public R CollectItem<R, C>(ref C colctor, Type<T> b = default) where C : ISeqSeraColctor<T, I, R>
        => colctor.CItem(d, new SeqIImmutableSetEffector<T, I>());
}

public readonly struct SeqIImmutableSetEffector<T, I> : ISeraEffector<T, I>
    where T : IImmutableSet<I>
{
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
