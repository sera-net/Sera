...
internal class _IEnumerable(Type ItemType) : _Generic(ItemType)
{
    ...
    public override Type GetEmitType(EmitStub stub, EmitMeta target, EmitDeps deps)
        => EmitType =
            typeof(SeqIEnumerableImpl<,,>).MakeGenericType(target.Type, ItemType, deps.Get(0).MakeSerWrapper(ItemType));
