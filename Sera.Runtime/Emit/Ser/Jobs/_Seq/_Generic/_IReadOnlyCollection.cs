...
internal class _IReadOnlyCollection(Type ItemType) : _Generic(ItemType)
{
    ...
    public override Type GetEmitType(EmitStub stub, EmitMeta target, EmitDeps deps)
        => EmitType =
            typeof(SeqIReadOnlyCollectionImpl<,,>).MakeGenericType(target.Type, ItemType,
                deps.Get(0).MakeSerWrapper(ItemType));
