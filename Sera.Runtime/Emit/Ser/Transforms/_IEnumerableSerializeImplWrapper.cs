...
internal class _ICollectionSerializeImplWrapper(Type ItemType) : EmitTransform
{
    public override Type TransformType(EmitMeta target, Type prevType)
    {
        return typeof(ICollectionSerializeImplWrapper<,>).MakeGenericType(target.Type, ItemType);
    }

    public override object TransformInst(EmitMeta target, Type type, Type prevType, object prevInst)
    {
        var ctor = type.GetConstructor(new[]
            { typeof(ICollectionSerializeImplBase<,>).MakeGenericType(target.Type, ItemType) })!;
        return ctor.Invoke(new[] { prevInst });
    }
}
